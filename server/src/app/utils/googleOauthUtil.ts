...
export const getGoogleUser = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Google userinfo failed: ${response.status} ${message}`);
    }
...
export const refreshGoogleAccessToken = async (refreshToken: string) => {
  try {
    //Setting credentials with refresh token
    oauthClient.setCredentials({ refresh_token: refreshToken });

    // Calling Google API to refresh token
    const { credentials } = await oauthClient.refreshAccessToken();

    if (!credentials.access_token) {
      // No access_token in response
      throw new Error(
        "Failed to refresh access token - no access_token received",
      );
    }

    // Successfully refreshed token
    console.log("Token expires at:", credentials.expiry_date);
